...
def Ram():
    ...
    if dict(psutil.virtual_memory()._asdict())["available"] >= 2857841664:
        return True
    else:
        return False
...
def Bitcoin_Price():
    ...
    return int(str(data).replace(",", "").split(".")[0])
...
def Capture_Photo():
    ...
    while(True):
        cv2.imshow('Image capturede from webcam',frame)
...
# Check the system and resources
...
Commands list:
    ...
    - Open any site
    - Empty recucle bin
    - Capture photo in moment from webcam
    - Search in wikipedia
    - Power 
        - Hibernate(PRO)
        - Shodown(PRO)
    ...
    - Voice
        ...
        - Voice gender(male and female)
        - Voice speed

enjoy winbloke 0.3.1 :)
...
while True:
    ...
    #     speak("your device will be shotdown in 3 seconds. to quit, press any button")

    #     with Listener(on_press=on_press) as listener:
    #         for i in range(3):
    #             if flag:
    #                 speak(str(3-i)+" seconds left")
    #                 time.sleep(0.5)
    #             else:
    ...
    #     speak("your device will be restart in 3 seconds. to quit, press any button")
    ...
    #     speak("your device will be lock in 3 seconds. to quit, press any button")
    ...
    #     speak("your device will be hibernate in 3 seconds. to quit, press any button")
    ...
    # elif match(command, APIs_Dict["weather"]): # done
    #     user_ip = Get_Ip()
    #     user_region = Ip_Country(user_ip)[1]
    #     speak("Weather by your ip region, "+user_region+" is")
    #     Weather(user_region)
    # elif match(command, APIs_Dict["joke"]): #joke
    #     Joke()
    # elif match(command, APIs_Dict["speedtest"]): #done
    #     Internet_Speedtest()
    # elif match(command, APIs_Dict["desktop_wallpaper"]): # done
    ...
    #     engine.setProperty('volume', volume)
    #     speak("voice volume is louder now")
    # elif match(command, APIs_Dict["bitcoin"]):
    #     price = str(Bitcoin_Price())
    #     speak("bitcoin price in moment is "+price+" USD.")
    # elif match(command, APIs_Dict["open_site"]): #done
    #     site_name = command.replace("open", "").replace("go to", "").replace(" ", "").replace("dot", ".")
    #     Open_Websites(site_name)
    #     speak(site_name+" opened.")
    # elif match(command, APIs_Dict["wikipedia"]): #done
    #     # search @!# in wikipedia
    #     # see @!# in wikipedia
    #     # what cand you find about @!# in wikipedia?
    #     subject = command.lower().replace("wikipedia", "").replace("see", "").replace("what can you find", "").replace("search", "").replace("in", "")

    #     speak("searching " + subject +" in wikipedia ...")
        
    #     text = Wikipedia_Search(subject)
    #     print(text)
    #     speak("by wikipedia: "+text)
    # elif match(command, APIs_Dict["capture_photo"]):
    #     speak("taking a photo from your webcam. press C button to close photo")
    #     Capture_Photo()
    # elif match(command, APIs_Dict["empty_recycle_bin"]):
    #     flag = True
    ...
    #     speak("your recycle bin will be empty in 3 seconds. to cancel it, press any button")
    ...
    #     if flag:
    #         Delete_Temprery_Files()
    #         speak("your recycle bin is empty now")
